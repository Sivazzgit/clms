...
if($_SESSION['UserID']<>'0000' ){
  ...
  exit("Not allowed. Unauthorised User");
    ...
    $sql1 = "select  distinct employeeid,area from clmsusersection where employeeid='".$_SESSION['UserID']."'";
...
/*  
  ...
  */  
  ...
  echo'<TD hidden><a href="usersectionupdation2.php">' . _('Select Section') . '</a></TD>';
...
//Next day of the Sdate calculation starts
$sqln='select STR_TO_DATE("'.$Sdate.'","%d-%m-%Y") as"dt1" from clmsshiftroster0222 limit 1';
  $resultn = DB_query($sqln,$db); 
$myrown = DB_fetch_row($resultn);
//echo"Next Day =".$myrown[0];

$sqln2="select ADDDATE('".$myrown[0]."', INTERVAL 1 day) as'dt2' from clmsshiftroster0222 limit 1";
  $resultn2 = DB_query($sqln2,$db); 
$myrown2 = DB_fetch_row($resultn2);
//echo"Next Day =".$myrown2[0];

$sqln3="select date_format('".$myrown2[0]."','%d-%m-%Y') as'dt3' from clmsshiftroster0222 limit 1";
  $resultn3 = DB_query($sqln3,$db); 
$myrown3 = DB_fetch_row($resultn3);

$nextday=$myrown3[0];

//$nextday=STR_TO_DATE('".$myrown2[0]."','%d-%m-%Y');
//echo"Next Day 1 =".$nextday;
//Next day of the Sdate calculation Ends here
...
if($mmonth1<>'1221'){
  $sql0="CREATE TABLE IF NOT EXISTS clmsmanpower".$mmonth1." LIKE  clmsmanpower1221" ;
  $result0 = DB_query($sql0,$db); 
}

if($mwc<>''){
  
  $sql0="CREATE temporary table tsec select section from clmsusersection where area='".$mwc."'" ;
  $result0 = DB_query($sql0,$db); 
}else{
  $sql0="CREATE temporary table tsec select section from clmsusersection " ;
  $result0 = DB_query($sql0,$db); 
  ...
  $result0 = DB_query($sql0,$db); 
//echo $sql0;
 // $sql0="create temporary table tareacheck select a.*  from   clmsmanpower".$mmonth1." a, tsec01 b where trim(a.employeeid)=trim(b.employeeid) and  a.dt between '".$Sdate."' and '".$Edate."' and trim(a.shift)='".$mshift."'";
 $sql0="create temporary table tareacheck select *  from   clmsmanpower".$mmonth1."  where dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."'";
  $result0 = DB_query($sql0,$db);  
...
//    $sql="select * from tareacheck where workcenter <>'$mwc'";  
 //   $sql="select workcenter as 'Done by',dt as 'Date',Shift,Employeename,Type,HRS,Grade,Actgrade,Section,SPICE,Manning,Timein,Sshift as'Sch Shift' from tareacheck where workcenter <>'$mwc'";  
     $sql="select workcenter as 'Done by',dt as 'Date',Shift,Employeename,Type,HRS,Section,Timein from tareacheck where workcenter <>'$mwc'";  
    ...
    $column_count = mysqli_num_fields($result03);
//      echo"Check-3";
 //         echo $sql; 
  ...
  echo "</tr><tr><th colspan='".($column_count)."'>";
    ...
    for($column_num = 0; $column_num < $column_count; $column_num++) 
    {
       // $field_name = mysql_field_name($result03, $column_num);
        $fieldName = mysqli_fetch_field_direct($result03, $column_num)->name;
  ...
  $sql0="select count(*) from   clmsmanpower".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."' and workcenter='".$mwc."'";
      $result0 = DB_query($sql0,$db); 
//echo $sql0;
  ...
  $sql = "CREATE TEMPORARY TABLE TempT1 as (SELECT *,00 as'slno'  FROM clmsmanpower".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."')";
    $result = DB_query($sql,$db);
...
$msln=1 ;
 while ($msln <10)  
  ...
  $sql = "CREATE TEMPORARY TABLE TempT1 as (SELECT * ,00 as'slno' FROM clmsmanpower".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."' and workcenter='".$mwc."')";
//echo $sql;
  $result = DB_query($sql,$db);
//    echo "rcd cnt=".$mcn;
    $msln=$mcn+1 ;
...
//  echo "Hai";
      ...
      $sql = "delete from clmspunching";
      $result = DB_query($sql,$db);

//      $sql = "insert into clmspunching(column0,column3,dt,time0,column6) select employeeid,in_out,date,(trim(time)/1),time  FROM clmspunchingapi where trim(date)= '".$Sdate."' and employeeid>10 and employeeid<200 ";
//      $sql = "insert into clmspunching(column0,column3,dt,time0,column6) select employeeid,in_out,date,(trim(time)/1),time  FROM clmspunchingapi where  employeeid>10 and employeeid<200 ";
//      $sql = "insert into clmspunching(column0,column3,dt,time0,column6) select employeeid,in_out,date,(trim(time)/1),time  FROM clmspunchingapi where  employeeid>10 and employeeid<200  and substr(trim(date),3,7)=substr(trim('".$Sdate."'),3,7)";
 //     $sql = "insert into clmspunching(column0,column2,column3,column6,dt,time0) select column0,column2,column3,column6,dt,time0  FROM clmspunchingupload where  column0>2000 and column0<3000  and (STR_TO_DATE(trim(dt),'%d-%m-%Y') >= STR_TO_DATE('".$Sdate."','%d-%m-%Y') )";
     $sql = "insert into clmspunching(column0,column2,column3,column6,dt,time0) select column0,column2,column3,column6,dt,time0 
	 FROM clmspunchingupload where  column0>2000 and column0<3000 
	 and  (STR_TO_DATE(trim(dt),'%d-%m-%Y') >= STR_TO_DATE('".$Sdate."','%d-%m-%Y'))
	 and  (STR_TO_DATE(trim(dt),'%d-%m-%Y') <= STR_TO_DATE('".$nextday."','%d-%m-%Y') )";
//echo $sql;
      $result = DB_query($sql,$db);
	 
	 //data fetching from punchingapi 
     $sql = "insert into clmspunching(column0,column2,dt,time0,column6) select employeeid,'1',date ,(trim(time)/1),time 
	 FROM punchingapi where  employeeid>2000 and employeeid<3000 and trim(terminal)='CJOY204260830'   
	 and  (STR_TO_DATE(trim(date),'%d-%m-%Y') >= STR_TO_DATE('".$Sdate."','%d-%m-%Y'))
	 and  (STR_TO_DATE(trim(date),'%d-%m-%Y') <= STR_TO_DATE('".$nextday."','%d-%m-%Y') )";
//echo $sql; 
 $result = DB_query($sql,$db);
 
      $sql = "insert into clmspunching(column0,column2,dt,time0,column6) select employeeid,'1',date ,(trim(time)/1),time 
	 FROM punchingapi where  employeeid>2000 and employeeid<3000 and trim(terminal)='CJOY204260837'  
	 and  (STR_TO_DATE(trim(date),'%d-%m-%Y') >= STR_TO_DATE('".$Sdate."','%d-%m-%Y'))
	 and  (STR_TO_DATE(trim(date),'%d-%m-%Y') <= STR_TO_DATE('".$nextday."','%d-%m-%Y') )";
      $result = DB_query($sql,$db);
 //echo $sql;  
 			$sql = "update  clmspunching set time0=substr(column6,1,2)+(substr(column6,4,2)/100) where trim(column6)<>''";
			$result = DB_query($sql,$db);


      $sql = "update  clmspunching set column0='74008' where column0='78004'"; 
      $result = DB_query($sql,$db);
    //  $sql = "update  clmspunching set time0=substr(column6,1,2)+(substr(column6,4,2)/100)"; 
    //  $result = DB_query($sql,$db);
include('punchingdateround.php'); 
      $sql = "update clmspunching a,clmsshiftroster".$mmonth1."  b  set a.column2=b.shift where trim(a.column0)=trim(b.employeeid) and trim(a.column4)=trim(b.dt)";  
//      $sql = "update clmspunching a,clmsshiftroster".$mmonth1."  b  set a.column2=b.shift where trim(a.column0)=trim(b.employeeid) and trim(a.dt)=trim(b.dt)";  
    //  $result = DB_query($sql,$db);
      $sql = "update clmspunching a,shiftchange  b  set a.column2=b.chshift where trim(a.column0)=trim(b.employeeid) and trim(a.column4)=trim(b.dt)";  
    //  $result = DB_query($sql,$db);
//echo $sql;
      $sql = "update  clmspunching set column3=''   ";
      $result = DB_query($sql,$db);

        
      $sql = "update  clmspunching set column3='IN' where column2='1' and column5 >5.00 and column5<7.00  ";
      $result = DB_query($sql,$db);

      $sql = "update  clmspunching set column3='OUT' where column2='1' and column5 >13.00 and column5<18.00  ";
      $result = DB_query($sql,$db);
      
      $sql = "update  clmspunching set column3='IN' where column2='2' and column5 >12.00 and column5<22.00  "; 
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column3='OUT' where column2='2' and column5 >21.00 and column5<27.00  ";
      $result = DB_query($sql,$db);

      $sql = "update  clmspunching set column3='IN' where column2='3' and column5 >20.00 and column5<26.00  ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column3='OUT' where column2='3' and column5 >27.00 and column5<33.00  ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column3='OUT' where column2='3' and column5 >5.00 and column5<9.00  ";
      $result = DB_query($sql,$db);

      $sql = "update  clmspunching set column3='OUT' where column2='O' and column5 >5.00 and column5<9.00  ";
      $result = DB_query($sql,$db);
      
       
      $sql = "update  clmspunching set column3='IN' where column2='1'   ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column3='OUT' where column2='2'   ";
      $result = DB_query($sql,$db);
...
//echo $sql;        
      ...
      $sql = "update  clmspunching set column2='A' where column2='1'   ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='B' where column2='2'   ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='C' where column2='3'   ";
      $result = DB_query($sql,$db);

      $sql = "update  clmspunching set column2='A' where column3='IN' and column5 >5.00     ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='B' where column3='IN' and column5 >13.00     ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='C' where column3='IN' and column5 >21.00     ";
      $result = DB_query($sql,$db);
      
      $sql = "update  clmspunching set column2='C' where column3='OUT' and column5 >21 and column5<31    ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='B' where column3='OUT' and column5 >16.00 and column5 < 26    ";
      $result = DB_query($sql,$db);
      $sql = "update  clmspunching set column2='A' where column3='OUT' and column5 >13.00 and column5 < 18    ";
      $result = DB_query($sql,$db);
 ...
 //echo $sql;     
  if($mcn ==0){
    ...
    $sql = "CREATE TEMPORARY TABLE tpunching select * from clmspunching where trim(column4)='".$Sdate."' and trim(column2)='".$mshift."' and trim(column3)='IN'";
//echo $sql;
      $result = DB_query($sql,$db);
...
/* 
  $sql = "select column0,column2,column3,column4,dt from  tpunching ";
   ...
   while ($myrow = DB_fetch_row($result))  
    {
  echo '<br> PPPPPPPP'.$myrow[0].'--'.$myrow[1].'--'.$myrow[2].'--'.$myrow[3].'--'.$myrow[4];
  //.'--'.$myrow[5];
  }
 */ 
...
//Not required to show absentees from shift rotation
//        $result = DB_query($sql,$db);
...
*/  
        $sql = "CREATE TEMPORARY TABLE TempT2 as (SELECT * ,00  as'slno',000.00 as'timeout' FROM manpower0721 limit 0)";
 $result = DB_query($sql,$db);
//echo $sql; 
//      $sql = "create temporary table Temp3 select  distinct a.column0 as'employeeid',b.lastname as'name',b.section,b.grade from clmspunching a, clmsemployeemaster b where a.column0=b.employeeid and a.dt between '".$Sdate."' and '".$Edate."' and trim(a.column2)='".$mshift."' and trim(a.column3)='IN'"; 
      $sql = "create temporary table Temp3 select  distinct a.column0 as'employeeid',b.lastname as'name',b.section,b.grade,a.time0 as'timein','                             'as'sect' from clmspunching a, clmsemployeemaster b where a.column0=b.employeeid and trim(a.column4) between '".$Sdate."' and '".$Edate."' and trim(a.column2)='".$mshift."' and trim(a.column3)='IN'"; 
 ...
 // echo $sql ;
     $result = DB_query($sql,$db); 
      ...
      $sql = "insert into TempT2(slno,employeeid,employeename,type,hrs,section,grade,timein) select 0,employeeid,name,'Duty',0.00,section,grade,timein from Temp3 where trim(sect) <>'1'"; 
//echo $sql; 
      $result = DB_query($sql,$db);
//echo $sql;	  
/*	
...
*/	
      $sql = "update TempT2 a,clmsemployeemaster b set slno=1 where a.employeeid=b.employeeid and trim(b.dept)='".$kkdept."'" ;  
      $result = DB_query($sql,$db);
            if($kkdept<>'ALL'){
        $sql = "delete from TempT2 where slno=0" ;  
        $result = DB_query($sql,$db);
            } 
        $sql = "update TempT2 set slno=0" ;  
        $result = DB_query($sql,$db);
...
//echo $sql;
      ...
      $sql = "CREATE TEMPORARY TABLE TempT2 as (SELECT * ,00 as'slno' FROM clmsmanpower".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."' and workcenter='".$mwc."')";
    $result = DB_query($sql,$db);
      $sql = "update TempT2 a,clmsshiftroster".$mmonth1."  b  set a.sshift=b.shift where a.employeeid=b.employeeid and b.dt='".$Sdate."'";  
//echo $sql;
      $result = DB_query($sql,$db);
...
$sql="update t3 set slno=num"; 
$result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql="update t3 a, clmspunching b set a.Timeout=b.time0 where trim(a.employeeid)=trim(b.column0) and trim(b.column4)='".$Sdate."' and b.column3='OUT'"; 
$result = DB_query($sql, $db, $ErrMsg, $DbgMsg);

//*****
$sql="create temporary table tpatt select *,0000.00 as'cinpunch',0000.00 as'coutpunch',0000.00 as'chrs',
00000.000 as'tchr',00000.00 as'tcmin',00000.00 as'eout','  ' as'pssh',00.00 as'roundhrs' from t3" ; 
$result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//Actual Hrs Calculation

$sql = "update tpatt set cinpunch=timein ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set cinpunch=6.00 where timein>5 and  timein <=6 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set cinpunch=9.00 where timein>9 and  timein <11 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//$sql = "update tpatt set cinpunch=9.00 where timein>0 and  timein <9 and pshift =4";
$sql = "update tpatt set cinpunch=9.00 where timein>7 and  timein <9 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set cinpunch=14.00 where timein>13 and  timein <16 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set cinpunch=22.00 where timein >20 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set cinpunch=24.00 where timein >=0 and  timein<5 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);


$sql = "update tpatt set coutpunch=timeout ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set coutpunch=14.00 where timeout>14 and timeout <=15";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set coutpunch=14.00 where timeout>14 and timeout <16";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//$sql = "update tpatt set coutpunch=17.30 where timeout>17.30 and   pshift =4";
 $sql = "update tpatt set coutpunch=17.30 where timeout>17.30  and timein <11";
       $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
if($mshift=='B'){
  $sql = "update tpatt set coutpunch=24.00 where timeout>=0 and timeout <5 ";
}else{
  $sql = "update tpatt set coutpunch=24.00 where timeout>0 and timeout <5 ";
} 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set coutpunch=32.00 where timeout>8 and timeout <10";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
        
$sql = "update tpatt set eout=coutpunch " ;
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//$sql = "update tpatt set eout=coutpunch where pshift =3";
//        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//$sql = "update tpatt set eout=32.00 where (cout+24)>32 ";
//        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//$sql = "update tpatt set eout=0 where cinpunch=0 ";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//

$sql = "update tpatt set chrs=(floor(eout)*60)+((eout-floor(eout))*100)-((floor(cinpunch)*60)+((cinpunch-floor(cinpunch))*100))   "; 
      $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set tchr=floor(chrs/60)";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
      

$sql = "update tpatt set tcmin=chrs-(tchr*60)";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
        
$sql = "update tpatt set chrs=tchr+(tcmin/100)";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set roundhrs=tchr "; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set roundhrs=tchr+.15 where tcmin>0"; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set roundhrs=tchr+.5 where tcmin>15"; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql = "update tpatt set roundhrs=tchr+.75 where tcmin>30";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
      
$sql = "update tpatt set roundhrs=tchr+1 where tcmin>45";
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);

$sql = "update tpatt set roundhrs=0 where roundhrs<0"; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
        
$sql = "update tpatt set roundhrs=8 where roundhrs > 8"; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
        
        
$sql = "update tpatt set chrs=0 where chrs<0"; 
        $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql="update t3 a, tpatt b set a.hrs = b.roundhrs where trim(a.employeeid)=trim(b.employeeid) and a.hrs=0";  
$result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//echo $sql;  
...
/*
 $sql="select employeeid,dt,Timein,Timeout from  t3 ";  
 $result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
 while ($myrow = DB_fetch_row($result))  
    {
    echo "<br>".$myrow[0].$myrow[1].$myrow[2].$myrow[3];
  }
...
$sql = "select employeeid,Employeename,sshift as'Sc.Shift',Type,HRS AS'Hours',Section as 'Working Section',Grade as'Grade',Actgrade,Spice,Manning,Timein,slno,yarea,Timeout from t3"; 
//echo $sql;
  ...
  echo "<FORM METHOD='post' id='f1' ACTION='gatemanpowerentry1.php'"; 
...
$mheading="Manpower Data Updation-".$kkdept;
...
//   echo'<br><span>Intend : </span>'.$mintend.'  <span>Present : </span>'.$mpresent.'<span> Absent : </span>'.$mabsent.'<span> Over Time : </span>'.$mextrawork;
   ...
   echo'<left><table id="tbl1" Border="1" width ="50%"  cellspacing="0" cellpaddin="0">'; 
$column_count=$column_count-0 ; 
  ...
  echo "<th colspan='".($column_count)."'>";
    ...
    {
      //  $field_name = mysql_field_name($result03, $column_num);
        $field_name = mysqli_fetch_field_direct($result03, $column_num)->name;
    
    if($field_name =='employeeid'){
           print("<TH  width='15%'>Employee ID</TH>\n") ;
    }else{
      
      if(($column_num>=2 and $column_num<=3) or ($column_num>=6 and $column_num<=9) or ($column_num>10 and $column_num<13) ){
...
/*      
 ...
 //          echo "<TD><input  type='text1' name='posi$mcount'  value='" . $myrow03[2] . "' readonly>";
           echo "<TD><center><input style='width:50%'  readonly type='text' name='empid$mcount'  value='" . $myrow03[0] . "'></center></td>"  ;
...
//         $sql = 'SELECT employeeid,lastname FROM tmp where employeeid>10 order by 1000000-employeeid desc';
         $sql = 'SELECT employeeid,lastname FROM tmp where employeeid>10 order by lastname';
      $result = DB_query($sql, $db);
  ...
  echo  "</TD></SELECT>  ";   
    ...
    //  $sql = 'SELECT employeeid,lastname FROM clmsemployeemaster where orgunit="CATL" and active=0';
        // $sql = 'SELECT distinct section  FROM usersection where employeeid="CRK1020"';
         $sql = 'SELECT distinct section  FROM clmssectionmaster order by section';
      $result = DB_query($sql, $db);
...
echo "<TD hidden><center><input style='width:50%'  id='man' type='text1' name='man$mcount' readonly value='" . $myrow03[9] . "'></center></td>"  ;
    ...
    echo "<TD><center><input readonly style='width:70%'  type='text1' name='timein$mcount'  value=" . $myrow03[10] . "></center></td>";
    echo "<TD><center><input readonly style='width:70%'  type='text1' name='timeout$mcount'  value=" . $myrow03[13] . "></center></td>";
...
//      echo "<TD>" . $mcount . "</TD>";
  ...
  //  echo "<TD><input  type='text1' name='cat$mcount'  value=" . $myrow03[6] . ">";
     ...
     //      echo "<TD><input  type='text1' name='typ$mcount'  value=" . $myrow03[8] . ">";
      ...
      echo "<INPUT TYPE='hidden' NAME='mintend' VALUE='".$mintend."'>";
    if(trim($myrow03[12])<>''){
      $msubmitted='Y';
  }
  } 
   echo'</table>';
    echo '</div>';
$cd=date('d-m-Y',strtotime("-10 days"));
if($Sdate<$cd){
