...
$title = _('Shift Wise Labour Engaged');
...
//$md0="update clmsengaged a, clmsapprovalmatrix b  set a.approver=trim(b.approver) where substr(trim(a.createdby),1,8)=trim(b.creater)";

$md0="update clmsengaged a, clmsapprovalmatrix b  set a.approver=trim(b.approver) where trim(a.section)=trim(b.section)";
...
$md0="update clmsengaged   set total=ashift+bshift+cshift";
...
$md="select * from clmsengaged order by recordid desc";

}

if(substr($_SESSION['UserID'],0,5)=='clms0' ){

$md="select * from clmsengaged where vendor='".$myrow[0]."'";

//$md="select * from clmsengaged where trim(vendor)='".$kkdept."' order by recordid desc";
...
$md="select a.* from  clmsengaged a,clmsusersection b where trim(a.section)=trim(b.section) and trim(b.employeeid)='".$_SESSION['UserID']."' order by recordid desc";
...
if(substr($_SESSION['UserID'],0,5)<>'clms0' ){		 
		...
		// '<br><br><center><a href="intendaddrecord.php">' . _('Add New Record') . '</a><BR>';
	...
	//echo '<center><a href="intendcalmonthly.php">' . _('Intend and Engagement View') . '</a><BR>';
		...
		echo '<br><br><center><a href="notyetkancor.php">' . _('Add New Record') . '</a><BR>';
...
echo'<center><H3>Shift Wise Labour Engaged</h3><center></td></tr>';
...
if(substr($_SESSION['UserID'],0,5)=='clms0' ){		 
		...
		echo '<TD><A HREF="'. $rootpath . '/addlabourengagementdata.php?' . SID . '&vendor='.$myrow1[1].'&section='.$myrow1[2].'&stdate='.$myrow1[3].'">' . _('Change') . '</A></TD>';
