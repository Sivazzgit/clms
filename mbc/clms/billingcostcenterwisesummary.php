...
/// echo $sql; 
	...
	$result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
 	$sql="insert into tsec0(section) select ''as 'section' from clmssectionmaster limit 1";   
	$result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
...
//	column2 LIKE CONCAT('%', column1, '%');	
 	...
 	$sql="create temporary table tmanp0 select a.* from clmsmanpower".$mlmonth." a, tsec0 b where substr(a.section,1,10)=substr(b.section,1,10) and a.category like '".$mvendor."%'";   
	$result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
//echo $sql;  
 	$sql="create temporary table tmanp1 select a.* from clmsmanpower".$mcmonth." a, tsec0 b where substr(a.section,1,10)=substr(b.section,1,10)  and a.category like '".$mvendor."%'";   
	$result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
...
$mheading='<center><H3> Cost Center wise Contract Laboutr Engagement for the Period from  '.$Sdate.' To '. $Todate.'- Vendor -'.$mvendor.' - Cost center '.$mcostcenter.'</h3></center>';
 $md="create temporary table tempt5 select section,sum(ashifteng) as'ashifteng',sum(bshifteng) as'bshifteng',sum(cshifteng) as'cshifteng',
 sum(totaleng) as'totaleng','xxxxxxxxx' as'crid','xxxxxxxx'as'aprid','xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' as'crnam',
'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' as'aprnam',costcenter from tempt4 group by section     ";
		$result = DB_query($md,$db);
$md="update tempt5 a,clmsapprovalmatrix b set a.crid=b.creater,a.aprid=b.approver where substr(a.section,1,10)=substr(b.section,1,10) ";
		$result = DB_query($md,$db);
$md="update tempt5 a,www_users b set a.crnam=b.realname where trim(a.crid)=trim(userid) ";
		$result = DB_query($md,$db);
$md="update tempt5 a,www_users b set a.aprnam=b.realname where trim(a.aprid)=trim(userid) ";
		$result = DB_query($md,$db);
$md="update tempt5 a,clmssectionmaster b set a.costcenter=b.costcenter where substr(a.section,1,10)=substr(b.section,1,10)  ";
		$result = DB_query($md,$db);

$md="update tempt5 set aprnam='' where substr(aprnam,1,4)='xxxx' ";
		$result = DB_query($md,$db);
$md="update tempt5 set crnam='' where substr(crnam,1,4)='xxxx' ";
		$result = DB_query($md,$db);
$md="update tempt5 set costcenter='No Sdection' where substr(costcenter,1,4)='xxxx' ";
		$result = DB_query($md,$db);
$md="delete from  tempt5 where  substr(section,1,4)='xxxx' and totaleng=0 ";
		$result = DB_query($md,$db);
		
$md="create temporary table tempt6 select section,sum(ashifteng) as'ashifteng',sum(bshifteng) as'bshifteng',sum(cshifteng) as'cshifteng',sum(totaleng) as'totaleng'  from tempt5  ";
		$result = DB_query($md,$db);
$md="update tempt6 set section='Total'  ";
		$result = DB_query($md,$db);
//$md="insert into tempt5(section,ashifteng,bshifteng,cshifteng,totaleng) select section,ashifteng,bshifteng,cshifteng,totaleng from tempt6  ";
//		$result = DB_query($md,$db);


$md="create temporary table tempt7 select Costcenter,Section ,sum(Ashifteng) as'Ashifteng',sum(Bshifteng) as'Bshifteng',sum(Cshifteng) as'Cshifteng',
sum(Totaleng) as'Totaleng','OUSMON LAMBHA' as'SO',crnam as 'SectionIncharge',aprnam as'PlantIncharge','Manu Wilson'as'HR' from tempt5 group by costcenter,section WITH ROLLUP" ;
 		$result = DB_query($md,$db);
$md="update tempt7 set section='CostCenter Total' where section='' and costcenter<>'' ";
		$result = DB_query($md,$db);
$md="update tempt7 set section='Grand Total' where section='' and costcenter='' ";
		$result = DB_query($md,$db);
$md="update tempt7 set Costcenter='Total' where section='Grand Total'  ";
		$result = DB_query($md,$db);
$md="update tempt7 set so='' where section<>'CostCenter Total'  ";
		$result = DB_query($md,$db);
$md="update tempt7 set hr='' where section<>'CostCenter Total'  ";
		$result = DB_query($md,$db);


$md="select * from tempt7 " ;
$md="select Costcenter,Ashifteng,Bshifteng,Cshifteng,Totaleng,SO,SectionIncharge,PlantIncharge,HR from tempt7  where Totaleng >0 "  ;
...
// $md="select Costcenter,Section ,sum(Ashifteng),sum(Bshifteng),sum(Cshifteng),sum(Totaleng),crnam as 'Section Incharge',aprnam as'Plant Incharge' from tempt5 group by costcenter,section WITH ROLLUP" ;
$md="select * from tempt7  where Totaleng >0 and so<>''"  ;
 
$md="select Costcenter,Ashifteng,Bshifteng,Cshifteng,Totaleng,SO,SectionIncharge,HR from tempt7  where Totaleng >0 and section='CostCenter Total' or section='Grand Total' "  ;
...
//echo '<tr>';
print("<TR>");
       if(trim($myrow1[1])<>'CostCenter Total'){
	//	echo '<TD><A HREF="'. $rootpath . '/billingdatewisesummarysection.php?' . SID . '&vendor='.$mvendor.' &section='.$myrow1[1].'&stdate='.$Sdate.'&endate='.$Todate.'&costcenter='.$mcostcenter.'">' . _('Show Details') . '</A></TD>';
	   }else{                                
	//	 echo'<TD></TD>';  
	   }   
		for($column_num = 0; $column_num < $column_count; $column_num++) {
