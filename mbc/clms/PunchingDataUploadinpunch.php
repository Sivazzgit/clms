...
//echo'rrrrr';
//$mmonth='0316';
echo '<left><a href="">' . _('Save the file name as data1.csv for uploading Machine -1 DATA') . '</a><BR>';
//echo '<left><a href="">' . _('Save the file as .csv format ') . '</a><BR>';

$sql="SELECT dt,time0 FROM clmspunchingupload where trim(column2)='1' order by  str_to_date(dt,'%d-%m-%Y') desc,time0 desc";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_array($result);
echo"<br><h3><center>Found Last In punch  upload done for ".$myrow[0]." at ".$myrow[1]."</center></h3><br>";
...
//echo $sql;
		...
		if(trim($filenam)<>'data1.csv'){
		  exit("<br>The file name must be as data1.csv for Machine -1 punch data  ");	
		}	
		$handle = fopen($file, "r");
		$c = 0;
	...
	//		$result = DB_query($sql,$db);
         ...
         
		{
			...
			{
				$name8= '0'.substr($name1,11,1);
    			$name9= substr($name1,13,2);
