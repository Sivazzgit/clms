...
//echo'rrrrr';
//$mmonth='0316';
echo '<left><a href="">' . _('Save the file name as data2.csv for uploading Machine -2 PUNCH DATA') . '</a><BR>';

$sql="SELECT dt,time0 FROM clmspunchingupload where trim(column2)='2' order by  str_to_date(dt,'%d-%m-%Y') desc,time0 desc";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_array($result);
echo"<br><h3><center>Found Last Out punch upload done for ".$myrow[0]." at ".$myrow[1]."</center></h3><br>";


echo'<P><left><INPUT Type="button" VALUE="Back" onClick="history.go(-2);return true;"><br>';
...
//echo $sql;
	    ...
	    $mout="Y";	
		if(trim($filenam)<>'data2.csv'){
		$mout="N";
		exit("<br>The file name must be as data2.csv for Machine -2  punch data  ");
		}	
		$handle = fopen($file, "r");
		$c = 0;
	...
	//		$result = DB_query($sql,$db);
         ...
         
		{
			...
			$name5 = $filesop[5];
			$name6 = $filesop[6];
			$name7= substr($name1,0,12);
			$name8= substr($name1,11,2);
			$name9= substr($name1,14,2);
		    if(strlen(trim($name1))<16)	
			{
