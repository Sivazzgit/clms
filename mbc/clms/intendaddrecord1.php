...
include('includes/headerclms.inc'); 
 echo'   <script src="script.js"></script>';
 	if (isset($_POST['submit'])) {
            ...
            $sql0 = 'SELECT count(*) from clmsintend where trim(vendor)="'.$_POST['vendor'].'" and trim(stdate)="'.$_POST['datepicker'].'"';
            $sql0 = 'SELECT count(*) from clmsintend where trim(vendor) like "'.$_POST['mvendor'].'%" and trim(section) like "'.$_POST['msection'].'%"and trim(stdate)="'.$_POST['mstdate'].'"';
...
//echo $sql0;
	...
	//	echo '<h3><br><br><center><a href="intend.php">' . _('Back to Intend Page') . '</a><BR></h3>';
		...
		$mcreatedby=$_SESSION['UserID'].' at '.$mmdt;
            ...
            $sql00 = 'insert into clmsintend(vendor,section,stdate,endate,ashift,bshift,cshift,urgent,createdby) values("'.$_POST['vendor'].'","'.$_POST['section'].'"
			,"'.$_POST['datepicker'].'",
...
//echo $sql00;		  
		...
		if(substr($_SESSION['UserID'],0,5)=='clms1' ){
           ...
           $sql00 = 'update clmsintend set endate="'.$_POST['datepicker1'].'" 
		                                    ,ashift='.$_POST["ashift"].'
										    ,bshift='.$_POST["bshift"].'
											,cshift='.$_POST["cshift"].'
