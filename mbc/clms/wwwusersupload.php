...
$title = _('Users  Data Upload');

//include('mfgheadermfg.php'); 
...
echo '<left><a href="hiring.csv">' . _('Users Creation Data Upload Format...') . '</a><BR>';
...
*/	
 ...
 $md="delete from  tupload where trim(column0)='EMP NO'  ";
  ...
  $md="update tupload set column19=concat(substr(column3,1,2),substr(column3,4,2),substr(column3,7,4))"; 
  //$md="update tupload set column19=trim(column2)";
					$result = DB_query($md,$db);
 $md2="select * from tupload  ";
 		$result2 = DB_query($md2,$db);
		while ($myrow2 = DB_fetch_row($result2)) {
			$mempno=trim($myrow2[1]);
				$md3="select count(*) from www_users where trim(userid)='".$mempno."'";
				$result3 = DB_query($md3,$db);
				while ($myrow3 = DB_fetch_row($result3)) {
                  if($myrow3[0]==0){   				 
				    echo"Record Not found in users master";
				    $md4="insert into www_users(userid) values('".$mempno."')";
					$result4 = DB_query($md4,$db);
					    echo"Record inserted in www_users userid=".$mempno;

				 
				   }else {
				  echo"Record Existes in www_users";
				  $md5 = "UPDATE www_users SET
					    realname='" . DB_escape_string(trim($myrow2[2])) . "',
				      	password='" . DB_escape_string(trim($myrow2[23])) . "',
				      	phone='" . DB_escape_string(trim($myrow2[8])) . "',
				      	email='" . DB_escape_string(trim($myrow2[9])) . "', 
				      	pagesize='A4',
				      	fullaccess=8,
						...
						WHERE trim(userid) = '".$mempno."' and trim(password) =''";
...
$md="	select userid,realname,password,phone,email,pagesize,fullaccess,modulesallowed,displayrecordsmax,language,theme,blocked from WWW_users";
 		$result = DB_query($md,$db);
$column_count = mysql_num_fields($result);
...
echo'<H3>Users  Master Data Updated </h3>';
echo'<center><table Border="1" width ="25%"  cellspacing="0" cellpaddin="0">';
