...
//         echo'tttt'.$EmployeeID1);  
 	  ...
 	  if($EmployeeID1>90000){
		$sql10="select lastname,position from clmsemployeemaster where employeeid='".$EmployeeID1."'";	
		}
		if($mtr=='T'){
...
//	$sql = "SELECT recordis,employeeid,date,time,in_out,terminal,DAYNAME(STR_TO_DATE(date,'%d-%m-%Y')) as'wkday' FROM punchingapi 	where employeeid ='".$EmployeeID1."'";
	//$sql = "SELECT recordid,column0,dt,time0,column2 from clmspunchingupload 	where column0='".$EmployeeID1."' order by STR_TO_DATE(trim(dt),'%d-%m-%Y') desc";
	$sql = "SELECT * from punchingapi where employeeid='".$EmployeeID1."' order by recordid desc";
if(trim($_POST['empno'])=='ALL'){ 
/*
	$sql = "SELECT * from punchingapi where employeeid>2000 and employeeid<3000 
	and STR_TO_DATE(trim(date),'%d-%m-%Y') >= 
    STR_TO_DATE(trim('".$_POST['stdate']."'),'%d-%m-%Y') and  STR_TO_DATE(trim(date)
	,'%d-%m-%Y') <= STR_TO_DATE(trim('".$_POST['endate']."'),'%d-%m-%Y')
 	order by recordid desc ";
*/
	$sql = "SELECT * from punchingapi where employeeid>2000 and employeeid<3000 
 	order by recordid desc ";

	//$sql = "SELECT * from clmspunchingupload  order by employeeid asc,recordid desc ";
}
...
echo '<CENTER><table border=1 id="tbl1" width ="60%" cellspacing="0" cellpaddin="0">';
 ...
 echo'<p><br>'.$mheading.'</p></th></tr>';
        ...
        echo "<tr>
		<td class='tableheader'>" . _('Recd.No ') . "</td>
		<td class='tableheader'>" . _('Employee ID ') . "</td>
		<td class='tableheader'>" . _('Date ') . "</td>
		<td class='tableheader'>" . _('Time ') . "</td>
		<td class='tableheader'>" . _('IN OUT ') . "</td>
		<td class='tableheader'>" . _('Terminal ') . "</td>
