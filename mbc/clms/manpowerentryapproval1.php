...
//	$sql = "CREATE TEMPORARY TABLE if not exists TempT2 as (SELECT *,00 as'slno'  FROM catlmfgdelayd".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and trim(shift)='".$mshift."' and trim(workcenter)='".$mwc."' )";
	$sql = "CREATE TEMPORARY TABLE if not exists TempT2 as (SELECT *,00 as'slno'  FROM clmsmanpowerap".$mmonth1." order by recordid limit 0 )";
  ...
  //  echo '<br>'.$mcount.'Details'.$_POST['empn'.$mcount];
	...
	// $memno= $_POST['empid'.$mcount];
 	 $memno= $_POST['empn'.$mcount];
		...
		workcenter = '".$mwc."',
		dt = '".$Edate."', 
		position = '".$_POST['posi'.$mcount]."',
		employeeid = '".$memno."',
		employeename = '".$_POST['empn'.$mcount]."',
		category = '".$_POST['cat'.$mcount]."',
...
$sql="update TempT2 a, clmsemployeemaster b set a.grade=b.grade where a.employeeid=b.employeeid ";
    $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
$sql="update TempT2 a, punching b set a.timein=b.time0 where a.employeeid=b.column0 and trim(a.timein)='' and a.dt=b.column4 ";
   $result03 = DB_query($sql, $db, $ErrMsg, $DbgMsg);
...
//************Allowance Config  Ends	
		...
		$sql0="select count(*) from 	clmsmanpowerap".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and shift='".$mshift."'";
//echo $sql0;
		$result0 = DB_query($sql0,$db); 
	...
	//if($mcn <> ($msln-1)){
	    ...
	    //echo "<br>msln".$msln;
	//	echo "Not tallied";
		$sql0="delete from 	clmsmanpowerap".$mmonth1." where  dt between '".$Sdate."' and '".$Edate."' and shift='".$mshift."' and workcenter='".$mwc."'";
	    $result0 = DB_query($sql0,$db); 
//		$sql0="insert into clmsmanpowerap".$mmonth1."(workcenter,shift,dt,employeeid,position,employeename,hrs,category,type,user) select workcenter,shift,dt,employeeid,position,employeename,hrs,category,type,'".$_SESSION['UserID']."'  from TempT2";
        $sql0="insert into clmsmanpowerap".$mmonth1."(dt,Workcenter,shift,Employeeid,Employeename,position,type,HRS,Section,Grade,Actgrade,Spice,Manning,Timein,sshift,category) select dt,Workcenter,shift,Employeeid,Employeename,position,type,HRS,Section,Grade,Actgrade,Spice,Manning,Timein,sshift,category from TempT2"; 
        $result0 = DB_query($sql0,$db);  
	 $currentDateTime = date('d-m-Y H:i:s');
     $mupdatedby=$_SESSION['UsersRealName'].$currentDateTime;
 // echo 'RRRRR'.$mupdatedby;	
		$sql0="update clmsmanpowerap".$mmonth1." set user='".$mupdatedby."' where  dt between '".$Sdate."' and '".$Edate."' and shift='".$mshift."' and workcenter='".$mwc."'";
	    $result0 = DB_query($sql0,$db); 
	
		
		if (isset($_POST['submit3'])){
		$sql0="update clmsmanpowerap".$mmonth1." set yarea='Submitted' where  dt between '".$Sdate."' and '".$Edate."' and shift='".$mshift."' and workcenter='".$mwc."'";
	    $result0 = DB_query($sql0,$db); 
...
$sql="select count(employeeid) from TempT2 where employeeid>0 and substr(type,1,2)='OT'";
   ...
   echo'<center><table id="tbl3" Border="1" width ="50%"  cellspacing="0" cellpaddin="0">'; 
	...
	echo "<tr><th colspan='3'><b>Contract Labour Engagement approval summary on ".$Sdate." in ".$mshift." Shift</b></th></tr>";
    print("<TR>");
	echo "<th>Present</th><th>Absent</th><th>Extra Work</th></TR>";
    echo"<tr><td><center>".$mpresent."</center></td><td><center>".$mabsent."</center></td><td><center>".$mextrawork."</center></td></tr></table>"; 
...
$sql = "select dt as'Date',Workcenter as'Plant',Shift,Employeeid,Employeename,Type,HRS,Section as 'Working Section',Grade as'Working Grade',Actgrade,Spice,Manning,Timein,sshift,slno from TempT2"; 
$sql = "select dt as'Date',Workcenter as'User',Shift,Employeeid,Employeename,Type,HRS,Section as 'Working Section',category as'Vendor',slno from TempT2"; 
...
$mheading="Contract Labour Engagement Data Approved";
